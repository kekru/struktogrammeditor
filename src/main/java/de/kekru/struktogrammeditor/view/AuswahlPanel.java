...
public class AuswahlPanel extends JPanel implements DropTargetListener, DragGestureListener, DragSourceListener {
	...
	private AuswahlPanelElement[] panelElemente = new AuswahlPanelElement[9]; // 9 StruktogrammElemente stehen zur
																				// Auswahl
	...
	public AuswahlPanel(Controlling controlling) {
		...
		muelleimerAuf(!muelleimerIstAuf); // dann mit ! einfuegen, weil in der Methode überprüft wird, ob er Parameter
											// != muelleimerIstAuf ist
	...
	public void dragGestureRecognized(DragGestureEvent evt) {// User hat angefangen ein Objekt zu ziehen
		...
		if (element instanceof AuswahlPanelElement) {// ist die Komponente ein AuswahlPanelElement (der User will ein
														// neues StruktogrammElement einfügen)...

			int typ = ((AuswahlPanelElement) element).gibTyp();// Typnummer für das entsprechende StruktogrammElement
																// ermitteln

			Transferable t = new StringSelection("n" + typ);// Drag-Daten setzen, n steht für neues Element, und typ ist
															// der Typ des neuen Elementes

			dragSource.startDrag(evt, DragSource.DefaultCopyDrop, t, this);// Drag wird ausgelöst

		} else if ((element == kopierFeld) && (kopiertesStrElement != null)) {// ist die Komponente das kopierFeld und
																				// ist ein Document in
																				// kopiertesStrElement abgelegt...
	...
	public void drop(DropTargetDropEvent event) {
		...
		try {
			...
			String dragTyp = (String) tr.getTransferData(tr.getTransferDataFlavors()[0]);// Drag & Drop Transferdaten
																							// ermitteln (das ist in
																							// diesem Programm ein
																							// Buchstabe und eventuell
																							// eine Zahl dahinter
																							// (siehe:
																							// Struktogramm.drop(...))
			...
			if (dragTyp.charAt(0) == 'z') {// z -> ein Drag wurde ausgelöst, indem ein StruktogrammElement aus dem
											// Struktogramm gezogen wurde

				if (dropUeberComponent == muelleimer) {
					// ein Element wurde aus dem aktuellen Struktogramm auf den Mülleimer gezogen ->
					// aus dem Zwischenlager des Struktogramms entfernen
					...
					// ein Element aus dem aktuellen Struktogramm wurde auf die Kopier-Box gezogen
					// -> xml-Abbild generieren und behalten
	...
	public void dragOver(DropTargetDragEvent evt) {
		Component tmp = getComponentAt(bildschirmKoordZuLokalenKoord(evt.getLocation())); // Komponente ermitteln, die
																							// unter der Maus ist

		muelleimerAuf(tmp == muelleimer); // wenn die Komponente unter der Maus das Mülleimer-Label ist, dann geöffneten
											// Mülleimer zeigen, sonst den Geschlossenen
		kopierFeldAuf(tmp == kopierFeld); // wie beim Mülleimer
	}

	// die Drag & Drop Methoden liefern Mauskoordinaten für den ganzen Bildschirm,
	// hier werden sie zu Koordinaten des AuswahlPanel konvertiert
	// siehe:
