...
public class Fallauswahl extends StruktogrammElement { // erbt von StruktogrammElement
	...
	// private Struktogramm str; //wird in hier gebraucht, um auf die Einstellung
	// zugreifen zu können, ob die letzten Elemente bei Bedarf gestreckt werden
	// sollen
	protected int xVerschiebungFuerTrennlinie; // legt fest, wie weit vom linken Rand der Fallauswahl die Trennlinie
												// zwischen Vorletztem- und Sonst-Fall sein soll
	protected int yVerschiebungFuerTrennLinie; // legt fest, wie hoch die oben genannte Trennlinie sein soll
	protected ArrayList<StruktogrammElementListe> listen; // Liste von StruktogrammElementListen, für die einzelnen
															// Fälle; als Generische ArrayList:
															// http://www.theserverside.de/java-generics-generische-methoden-klassen-und-interfaces/
	private int oldy = 0;
	...
	public Fallauswahl(Graphics2D g, int anzahlListen) {
		...
		yVerschiebungFuerTrennLinie = -20; // Trennlinie zwischen Vorletztem und Sonst-Fall geht 20 Pixel in den
											// Kopfteil hinein
	...
	public void quellcodeGenerieren(SupportedLanguages typ, int anzahlEingerueckt, int anzahlEinzuruecken,
		...
		switch (typ) {
		case Java:
			vorher = "switch(" + co("kommentar") + co("text") + co("kommentarzu") + "){\n";// Kopfteil besteht aus
																							// "switch(" plus
																							// eventuellem
																							// Kommentarzeichen plus die
																							// Textzeilen plus
																							// eventuellem Kommentar-Zu
																							// Zeichen plus "){"
			nachher = "}\n";
			break;
		...
		textarea.hinzufuegen(wandleZuAusgabe(vorher, typ, anzahlEingerueckt, alsKommentar)); // Kopfteil ausgeben, alles
																								// passend eingerückt
																								// und Kommentare bei
																								// Bedarf eingefügt
		...
		for (int i = 0; i < listen.size(); i++) {// Listen durchgehen
			...
			case Java:
				if (i < listen.size() - 1) {
					...
					fall = "default: " + co("zwangkommentar") + listen.get(i).gibBeschreibung() + co("zwangkommentarzu")
							+ "\n";// Sonsts-Fall erhält default, Beschriftung ist nicht relevant für fertigen
									// Code, also auf jeden Fall Kommentare setzen
				}
			...
			textarea.hinzufuegen(wandleZuAusgabe(fall, typ, anzahlEingerueckt + anzahlEinzuruecken, alsKommentar));// Anfang
																													// für
																													// den
																													// aktuellen
																													// Fall
																													// ausgeben
			listen.get(i).quellcodeAllerUnterelementeGenerieren(typ, anzahlEingerueckt + anzahlEinzuruecken * 2,
					anzahlEinzuruecken, alsKommentar, textarea);// Unterelemente ausgeben
			textarea.hinzufuegen(wandleZuAusgabe(fallEnde, typ, anzahlEingerueckt + anzahlEinzuruecken, alsKommentar));// Ende
																														// für
																														// den
																														// Fall
																														// ausgeben
		...
		textarea.hinzufuegen(wandleZuAusgabe(nachher, typ, anzahlEingerueckt, alsKommentar));// Ende der Fallauswahl
																								// ausgeben
	...
	// die Spalte mit der Nummer spaltenIndex wird nach links oder nach rechts
	// verschoben, je nach Parameter-Wert
	...
	public void entferneSpalte(int index) {
		if ((index >= 0) && (index < listen.size()) && (listen.size() > 2)) {// wenn mehr als 2 Spalten da sind, darf
																				// eine gelöscht werden
			listen.remove(index);
		}
	}
	...
	protected void zusaetzlicheXMLDatenSchreiben(Element aktuelles) {
		...
		for (int i = 0; i < listen.size(); i++) {
			unterelement = new Element("fall").setAttribute("fallname",
					XMLLeser.encodeS(listen.get(i).gibBeschreibung()));// für jeden Fall wird ein neuer fall-Tag
																		// generiert mit dem Fallnamen als Attribut
																		// (codiert)

			listen.get(i).schreibeXMLDatenAllerUnterElemente(unterelement);// in den neuen fall-Tag werden die xml-Daten
																			// der Unterelemente geschrieben
			aktuelles.addContent(unterelement);// fall-Tag wird in den strelem-Tag geschrieben, der zu dieser
												// Fallauswahl gehört
		}

	}

	// prüft, ob eventuellesUnterelement irgendwo innerhalb einer der Fall-Listen
	// steht
	@Override
	...
	public boolean neuesElementMussOberhalbPlatziertWerden(int y) {
		return y < gibY() + getObererRand() / 2;// ist die Maus im oberen Teil des Kopfteils, so soll ein neues
												// StruktogrammElement oberhalb eingefügt werden, sonst unterhalb
	}
	...
	@Override
	public void setzeBreite(int neueBreite) {// FallAuswahl wird horizontal getreckt, einzelne Fall-Listen müssen
												// mitgestreckt werden
		int gesamtbreiteDerListen = 0;
		int neueSpaltenbreite = 0;
		for (int i = 0; i < listen.size(); i++) {
			listen.get(i).xPosAllerUnterelementeSetzen(gibX() + gesamtbreiteDerListen);// x-Position der Unterelemente
																						// neu setzen
			if (i <= listen.size() - 2) {
				// Rechnung: neueGesamtbreite / alteGesamtbreite = neueSpaltenbreite /
				// alteSpaltenbreite //-> Verhältnisse sollen gleich bleiben
				// <=> neueSpaltenbreite = neueGesamtbreite * alteSpaltenbreite / alte
				// Gesamtbreite
	...
	protected Object gibElementAnPos (int x, int y, boolean nurListe) {
		...
		if (bereich.contains(x, y)) { // der angegebene Punkt ist in meinem Bereich
			for (int i = 0; i < listen.size(); i++) {
				...
				if (objGesetzt(tmp)) {
					return tmp; // eine Liste oder ein StruktogrammElement wurde gefunden, welches den Punkt
								// enthält
				}
			}

			if (!nurListe) {
				return this; // der Punkt ist nicht in einem der Fall-Listen, also, wenn nicht nur nach
								// Listen gefragt ist, diese Fallauswahl zurückgeben
			}
		}

		return null; // der Punkt ist nicht auf dieser Fallauswahl, oder er ist auf dem Kopfteil und
						// nurListe ist true
	}
	...
	public StruktogrammElementListe gibListeDieDasElementHat (StruktogrammElement element) {
		...
		for (int i = 0; i < listen.size(); i++) {// Fall-Listen werden gefragt, ob sie, oder deren Unterlisten, das
													// Element haben
	...
	private int gibPassendeYKoordFuerLinie (int x) {
		// herausfinden, bei welcher y-Koordinate die senkrechten Linien der Fallauswahl
		// oben enden müssen
		// y = m * x + b
		// m = (y2 - y1)/(x2-x1)
		double m = (double) (gibY() - (gibY() + getObererRand() + yVerschiebungFuerTrennLinie))
				/ (double) (gibX() - (gibX() + xVerschiebungFuerTrennlinie));
	...
	public void zeichne () {
		...
		g.drawLine(gibX(), gibY(), gibX() + xVerschiebungFuerTrennlinie,
				gibY() + getObererRand() + yVerschiebungFuerTrennLinie);
		g.drawLine(gibX() + xVerschiebungFuerTrennlinie, gibY() + getObererRand() + yVerschiebungFuerTrennLinie,
				gibX() + gibBreite(), gibY());

		// senkrechte Striche im Kopfteil der Fallauswahl und Fallbeschriftungen
		// zeichnen
		int x;
		int margin = 5;
		StruktogrammElementListe tmp;
		...
		for (int i = 0; i < listen.size(); i++) {
			...
			if (i != listen.size() - 1) {// senkrechte Striche zeichnen, die Sonstliste (die Letzte) braucht keinen
											// senkrechten Strich
			...
			if (this instanceof Verzweigung) {// ein bischen rumtricksen... bei der Verzweigung soll Ja und Nein ja ganz
												// an den Rändern stehen
				if (i == 0) {
					x = gibX() + margin; // 5 Pixel von linken Rand entfernt
				} else {
					x = tmp.gibRechterRand() - margin - gibTextbreite(tmp.gibBeschreibung()); // 5 Pixel vom rechten
																								// Rand entfernt
				}
	...
	public Rectangle zeichenbereichAktualisieren(int x, int y) {
		...
		for (int i = 0; i < listen.size(); i++) {
			...
			Rectangle rectListe = listen.get(i).zeichenbereichAllerElementeAktualisieren(x + gesamtbreiteDerListen,
					y + getObererRand());// Zeichenbereich der einzelnen Listen aktualisieren, die x-Position ist immer
											// der rechte Rand der vorherigen Liste
		...
		xVerschiebungFuerTrennlinie = gesamtbreiteDerListen - listen.get(listen.size() - 1).gibBreite(); // xVerschiebung
																											// der
																											// Trennlinie
																											// ist
																											// Breite
		...
		if (GlobalSettings.gibLetzteElementeStrecken()) {// wenn der User diese Einstellung gewählt hat...
			setzeHoehe(getObererRand() + groessteHoeheDerListen);// ...Höhe setzen und somit das letzte Element jeder
																	// Liste strecken
		}

		bereich.setBounds(x, y, gesamtbreiteDerListen, getObererRand() + groessteHoeheDerListen);// eigenen Bereich
																									// festhalten
